...
"""Реализация структурного паттерна Фасад
   ...
   В нашем случае мы вместо логики Framework'a печатаем сообщения.  
"""
class DebugFramework(Framework):

    def __init__(self, routes_obj, fronts_obj):
        self.framework = Framework(routes_obj, fronts_obj)
        super().__init__(routes_obj, fronts_obj)

    def __call__(self, environ, start_response):
        print('!! DEBUG MODE !!')
        print(environ)
        return self.framework(environ, start_response)
...
class FakeFramework(Framework):
    ...
    def __call__(self, environ, start_response):
        ...
        return [b'Ha-Ha-Ha! Fake is running!']
